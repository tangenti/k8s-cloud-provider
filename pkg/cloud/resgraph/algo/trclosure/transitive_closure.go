...
// state.
func OnGetFunc(f func(n resgraph.Node) error) Option {
	return func(c *Config) { c.onGet = f }
}

// Config for the algorithm.
type Config struct {
	onGet func(n resgraph.Node) error
}
...
func Do(ctx context.Context, cl cloud.Cloud, gr *resgraph.Graph, opts ...Option) error {
	klog.Infof("TransitiveClosure")

	config := Config{
		onGet: func(resgraph.Node) error { return nil },
	}
	for _, o := range opts {
		o(&config)
	}
	...
	for !work.Empty() {
		...
		if node.Ownership() == resgraph.OwnershipExternal {
		...
		for _, ref := range outRefs {
			...
			refNode, err := resgraph.NewNodeByID(ref.To, resgraph.NodeAttributes{})
			if err != nil {
				return fmt.Errorf("TransitiveClosure: %w", err)
			}
