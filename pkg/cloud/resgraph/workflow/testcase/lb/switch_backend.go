...
func init() {
	...
	graph2 := func() *resgraph.Graph {
		...
		mbs1.Access(func(x *compute.BackendService) {
			x.Backends = append(x.Backends, &compute.Backend{
				Group: b.N("neg-other").DefaultZone().NetworkEndpointGroup().SelfLink(),
			})
			x.HealthChecks = []string{
				b.N("hc").HealthCheck().SelfLink(),
			}
		...
		// NEG
		mneg1 := b.N("neg-other").DefaultZone().NetworkEndpointGroup().Resource()
		neg1, _ := mneg1.Freeze()
		graph.AddNetworkEndpointGroup(neg1, na).SetState(resgraph.NodeExists)
	...
	testcase.Register(&testcase.TestCase{
		Name:        "lb/switch-backend",
		Description: "Switch from one backend to another in UrlMap.",
		Steps: []testcase.Step{
			{Graph: graph1()},
			{Graph: graph2()},
			{Graph: graph2()},
