...
import (
	...
	"github.com/GoogleCloudPlatform/k8s-cloud-provider/pkg/cloud/resgraph/algo/traversal"
...
func (pl *planner) plan(ctx context.Context) (*Result, error) {
	...
	err := trclosure.Do(ctx, pl.cloud, pl.got,
		trclosure.OnGetFunc(func(n resgraph.Node) error {
			n.SetOwnership(resgraph.OwnershipManaged)
			return nil
		}),
	)
	if err != nil {
	...
	for _, gotNode := range pl.got.All() {
		...
		case gotNode.Ownership() == resgraph.OwnershipExternal:
			// TODO: clone the node from the "got" graph for "want" unchanged.
		case gotNode.Ownership() == resgraph.OwnershipManaged:
			// Nodes that are no longer referenced should be deleted.
			wantNode := gotNode.NewEmptyNode()
			wantNode.SetState(resgraph.NodeDoesNotExist)
...
func (pl *planner) propagateRecreates() error {
	var recreateNodes []resgraph.Node
	for _, node := range pl.want.All() {
		if node.LocalPlan().Op() == resgraph.OpRecreate {
	...
	for _, node := range recreateNodes {
		for _, inRefNode := range traversal.TransitiveInRefs(pl.want, node) {
			...
			if inRefNode.Ownership() != resgraph.OwnershipManaged {
	...
	for _, node := range pl.want.All() {
		...
		case resgraph.OpDelete:
			...
			for _, refs := range node.InRefs() {
				...
				} else if inNode.LocalPlan().Op() != resgraph.OpDelete {
